...
function! WinceCommonGetCursorPosition()
    call EchomLog('wince-common', 'debug', 'WinceCommonGetCursorPosition')
    return {
...
function! WinceCommonUberwinGroupExistsInState(grouptypename)
    call EchomLog('wince-common', 'debug', 'WinceCommonUberwinGroupExistsInState ', a:grouptypename)
...
function! WinceCommonSubwinGroupExistsInState(supwinid, grouptypename)
    call EchomLog('wince-common', 'debug', 'WinceCommonSubwinGroupExistsInState ', a:supwinid, ':', a:grouptypename)
...
function! WinceCommonUberwinGroupDimensionsMatch(grouptypename)
    call EchomLog('wince-common', 'debug', 'WinceCommonUberwinGroupDimensionsMatch ', a:grouptypename)
    for typename in WinceModelUberwinTypeNamesByGroupTypeName(a:grouptypename)
        call EchomLog('wince-common', 'verbose', 'Check uberwin ', a:grouptypename, ':', typename)
        let mdims = WinceModelUberwinDimensions(a:grouptypename, typename)
        if mdims.nr ==# -1 || mdims.w ==# -1 || mdims.h ==# -1
            call EchomLog('wince-common', 'debug', 'Uberwin ', a:grouptypename, ':', typename, ' has dummy dimensions')
            return 0
        endif
       ...
       \})
        ...
        if sdims.nr !=# mdims.nr || sdims.w !=# mdims.w || sdims.h !=# mdims.h
            call EchomLog('wince-common', 'debug', 'Uberwin ', a:grouptypename, ':', typename, ' has inconsistent non-dummy dimensions')
            return 0
        endif
    endfor
    call EchomLog('wince-common', 'debug', 'No dimensional inconsistency found for uberwin group ', a:grouptypename)
    return 1
endfunction
...
function! WinceCommonSupwinDimensionsMatch(supwinid)
    call EchomLog('wince-common', 'debug', 'WinceCommonSupwinDimensionsMatch ', a:supwinid)
    let mdims = WinceModelSupwinDimensions(a:supwinid)
    if mdims.nr ==# -1 || mdims.w ==# -1 || mdims.h ==# -1
        call EchomLog('wince-common', 'debug', 'Supwin ', a:supwinid, ' has dummy dimensions')
        return 0
    endif
    let sdims = WinceStateGetWinDimensions(a:supwinid)
    if sdims.nr !=# mdims.nr || sdims.w !=# mdims.w || sdims.h !=# mdims.h
        call EchomLog('wince-common', 'debug', 'Supwin ', a:supwinid, ' has inconsistent non-dummy dimensions')
        return 0
    endif
    call EchomLog('wince-common', 'debug', 'No dimensional inconsistency found for supwin ', a:supwinid)
    return 1
endfunction
...
function! WinceCommonSubwinGroupDimensionsMatch(supwinid, grouptypename)
    call EchomLog('wince-common', 'debug', 'WinceCommonSubwinGroupDimensionsMatch ', a:supwinid, ':', a:grouptypename)
    for typename in WinceModelSubwinTypeNamesByGroupTypeName(a:grouptypename)
        call EchomLog('wince-common', 'verbose', 'Check subwin ', a:supwinid, ':', a:grouptypename, ':', typename)
        let mdims = WinceModelSubwinDimensions(a:supwinid, a:grouptypename, typename)
        if mdims.relnr ==# 0 || mdims.w ==# -1 || mdims.h ==# -1
            call EchomLog('wince-common', 'debug', 'Subwin ', a:supwinid, ':', a:grouptypename, ':', typename, ' has dummy dimensions')
            return 0
        endif
        let winid = WinceModelIdByInfo({
       \    'category':'subwin',
       ...
       \})
        ...
        if sdims.nr !=# snr + mdims.relnr || sdims.w !=# mdims.w || sdims.h !=# mdims.h
            call EchomLog('wince-common', 'debug', 'Subwin ', a:supwinid, ':', a:grouptypename, ':', typename, ' has inconsistent non-dummy dimensions')
            return 0
        endif
    endfor
    call EchomLog('wince-common', 'debug', 'No dimensional inconsistency found for subwin group ', a:supwinid, ':', a:grouptypename)
    return 1
endfunction

" Get the window ID of the topmost leftmost supwin
function! WinceCommonFirstSupwinId()
    call EchomLog('wince-common', 'debug', 'WinceCommonFirstSupwinId')
    let minsupwinnr = 0
    let minsupwinid = 0
    for supwinid in WinceModelSupwinIds()
        if !WinceStateWinExists(supwinid)
            call EchomLog('wince-common', 'verbose', 'Skipping non-state-present supwin ', supwinid)
            continue
        endif
        call EchomLog('wince-common', 'verbose', 'Check supwin ', supwinid)
        let winnr = WinceStateGetWinnrByWinid(supwinid)
        if minsupwinnr ==# 0 || minsupwinnr > winnr
            call EchomLog('wince-common', 'verbose', 'Supwin ', supwinid, ' has lowest winnr so far: ', winnr)
            let minsupwinnr = winnr
            let minsupwinid = supwinid
        endif
    endfor
    call EchomLog('wince-common', 'debug', 'Supwin with lowest winnr is ', minsupwinid)
    return minsupwinid
endfunction
...
function! WinceCommonFirstUberwinInfo()
    call EchomLog('wince-common', 'debug', 'WinceCommonFirstUberwinInfo')
    let grouptypenames = WinceModelShownUberwinGroupTypeNames()
    if empty(grouptypenames)
        call EchomLog('wince-common', 'debug', 'No uberwins are open')
    ...
    call EchomLog('wince-common', 'debug', 'Selected uberwin ', grouptypename, ':', typename, ' as first uberwin')
    return {
   \    'category':'uberwin',
...
function! WinceCommonReselectCursorWindow(oldpos)
    call EchomLog('wince-common', 'debug', 'WinceCommonReselectCursorWindow ', a:oldpos)
    let pos = a:oldpos
   ...
   \   WinceModelSubwinGroupIsHidden(pos.supwin, pos.grouptype))
        call EchomLog('wince-common', 'debug', 'Cursor is in nonexistent or hidden subwin. Attempt to select its supwin: ', pos.supwin)
    ...
    if pos.category ==# 'none' || pos.category ==# 'supwin'
        call EchomLog('wince-common', 'debug', 'Validate winid ', pos.id, ' by model lookup')
   ...
   \   (pos.category ==# 'uberwin' && WinceModelUberwinGroupIsHidden(pos.grouptype))
        call EchomLog('wince-common', 'debug', 'Cursor position fallback to first supwin')
        let firstsupwinid = WinceCommonFirstSupwinId()
        if firstsupwinid
            call EchomLog('wince-common', 'debug', 'First supwin is ', firstsupwinid)
    ...
    if pos.category ==# 'none' ||
   \   (pos.category ==# 'supwin' && !WinceModelSupwinExists(pos.id))
        call EchomLog('wince-common', 'debug', 'Cursor position fallback to first uberwin')
    ...
    " state.
    call EchomLog('wince-common', 'debug', 'Cursor position selected based on model: ', pos)
    ...
    if !winexistsinstate && pos.category ==# 'subwin'
        call EchomLog('wince-common', 'debug', 'Cursor position is a state-closed subwin. Attempt to select its supwin: ', pos.supwin)
        let pos = {'category':'supwin','id':pos.supwin}
        let winexistsinstate = WinceStateWinExists(WinceModelIdByInfo(pos))
    endif
    ...
    if !winexistsinstate && pos.category ==# 'supwin'
        call EchomLog('wince-common', 'debug', 'Cursor position is a state-closed supwin. Fallback to first supwin')
        let firstsupwinid = WinceCommonFirstSupwinId()
        if firstsupwinid
            call EchomLog('wince-common', 'debug', 'First supwin is ', firstsupwinid)
            let pos = {'category':'supwin','id':firstsupwinid}
    ...
    call EchomLog('wince-common', 'verbose', 'Reselected cursor position ', pos)
    return pos
endfunction
...
function! WinceCommonShieldAllWindows(excludedwinids)
    call EchomLog('wince-common', 'debug', 'WinceCommonShieldAllWindows')
    ...
    for winid in WinceStateGetWinidsByCurrentTab()
        ...
        call EchomLog('wince-common', 'verbose', 'Shield ', winid, ' ' . onlyscroll)
        let preshield[winid] = WinceStateShieldWindow(winid, onlyscroll)
    endfor
    
    call EchomLog('wince-common', 'verbose', 'Shielded: ', preshield)
    return preshield
endfunction

" Unshield windows that were shielded with WinceCommonShieldAllWindows()
function! WinceCommonUnshieldWindows(preshield)
    call EchomLog('wince-common', 'debug', 'WinceCommonUnshieldWindows')
    for winid in keys(a:preshield)
        if WinceStateWinExists(winid)
            call EchomLog('wince-common', 'verbose', 'Unshield ', winid, ':', a:preshield[winid])
            call WinceStateUnshieldWindow(winid, a:preshield[winid])
        else
            call EchomLog('wince-common', 'verbose', 'Window ', winid, ' does not exist in state and so cannot be unshielded')
        endif
    endfor
endfunction
...
function! WinceCommonRestoreCursorPosition(info)
    call EchomLog('wince-common', 'debug', 'WinceCommonRestoreCursorPosition ', a:info)
    let newpos = WinceCommonReselectCursorWindow(a:info.win)
    call EchomLog('wince-common', 'debug', 'Reselected cursor position: ', newpos)
...
function! WinceCommonCloseUberwinsByGroupTypeName(grouptypename)
    call EchomLog('wince-common', 'debug', 'WinceCommonCloseUberwinsByGroupTypeName ', a:grouptypename)
    ...
    try
        ...
        call EchomLog('wince-common', 'debug', 'State-closing uberwin group ', a:grouptypename)
...
function! WinceCommonPreserveDimensions()
    call EchomLog('wince-common', 'debug', 'WinceCommonPreserveDimensions')
    ...
    call EchomLog('wince-common', 'verbose', 'Preserved dimensions: ', windims)
    return windims
endfunction
...
function! s:RestoreDimensionsByWinid(winid, olddims, prefertopleftdividers)
    call EchomLog('wince-common', 'verbose', 'RestoreDimensionsByWinid ', a:winid, ' ', a:olddims, ' ', a:prefertopleftdividers)
    ...
    if a:olddims.w <# newdims.w || (isuberwin && fixed.w && a:olddims.w ># newdims.w)
        call EchomLog('wince-common', 'debug', 'Set width for window ', a:winid, ' to ', a:olddims.w)
    ...
    if a:olddims.h <# newdims.h || (isuberwin && fixed.h && a:olddims.h ># newdims.h)
        call EchomLog('wince-common', 'debug', 'Set height for window ', a:winid, ' to ', a:olddims.h)
...
function! WinceCommonRestoreDimensions(windims)
    call EchomLog('wince-common', 'debug', 'WinceCommonRestoreDimensions')
    let sorted = copy(keys(a:windims))
    call sort(sorted, function('s:CompareWinidsByWinnr'))
    for winid in sorted
        if !WinceStateWinExists(winid)
            call EchomLog('wince-common', 'verbose', 'Window ', winid, ' no longer exists')
...
function! WinceCommonRecordAllDimensions()
    call EchomLog('wince-common', 'debug', 'WinceCommonRecordAllDimensions')
    ...
    for grouptypename in WinceModelShownUberwinGroupTypeNames()
        try
            ...
            let dims = WinceStateGetWinDimensionsList(winids)
            call EchomLog('wince-common', 'debug', 'Write state dimensions of uberwin group ', grouptypename, ' to model: ', dims)
            call WinceModelChangeUberwinGroupDimensions(grouptypename, dims)
        catch /.*/
            call EchomLog('wince-common', 'warning', 'WinceCommonRecordAllDimensions found uberwin group ', grouptypename, ' inconsistent:')
            call EchomLog('wince-common', 'debug', v:throwpoint)
            call EchomLog('wince-common', 'warning', v:exception)
        endtry
    endfor
    ...
    for supwinid in WinceModelSupwinIds()
        try
            ...
            call EchomLog('wince-common', 'debug', 'Write state dimensions of supwin ', supwinid, ' to model: ', dim)
            call WinceModelChangeSupwinDimensions(supwinid, dim.nr, dim.w, dim.h)
        catch
            call EchomLog('wince-common', 'warning', 'WinceCommonRecordAllDimensions found supwin ', supwinid, ' inconsistent:')
            call EchomLog('wince-common', 'debug', v:throwpoint)
            call EchomLog('wince-common', 'warning', v:exception)
        endtry
    ...
    " Record all subwin dimensions in the model
        ...
        for grouptypename in WinceModelShownSubwinGroupTypeNamesBySupwinId(supwinid)
            try
                ...
                call EchomLog('wince-common', 'debug', 'Write state dimensions of subwin group ', supwinid, ':', grouptypename, ' to model: ', dims)
                call WinceModelChangeSubwinGroupDimensions(supwinid, grouptypename, dims)
            catch /.*/
                call EchomLog('wince-common', 'warning', 'WinceCommonRecordAllDimensions found subwin group ', grouptypename, ' for supwin ', supwinid, ' inconsistent:')
                call EchomLog('wince-common', 'debug', v:throwpoint)
                call EchomLog('wince-common', 'warning', v:exception)
            endtry
        endfor
...
function! WinceCommonOpenUberwins(grouptypename, correctdims)
    call EchomLog('wince-common', 'debug', 'WinceCommonOpenUberwins ', a:grouptypename, ' ', a:correctdims)
    ...
    endtry
    call EchomLog('wince-common', 'verbose', 'Opened uberwin group ', a:grouptypename, ' with winids ', winids)
    return winids
endfunction
...
function! WinceCommonCloseUberwinsWithHigherPriority(priority)
    call EchomLog('wince-common', 'debug', 'WinceCommonCloseUberwinsWithHigherPriority ', a:priority)
    ...
    for grouptypename in reversegrouptypenames
        call EchomLog('wince-common', 'verbose', 'Uberwin group ', grouptypename, ' has higher priority')
...
function! WinceCommonReopenUberwins(preserved, correctdims)
    call EchomLog('wince-common', 'debug', 'WinceCommonReopenUberwins ', a:correctdims)
    ...
    for grouptype in a:preserved
        call EchomLog('wince-common', 'debug', 'Reopening preserved uberwin group ', grouptype.grouptypename)
        try
            let winids[grouptype.grouptypename] = WinceCommonOpenUberwins(grouptype.grouptypename, a:correctdims)
            call EchomLog('wince-common', 'verbose', 'Reopened uberwin group ', grouptype.grouptypename, ' with winids ', winids)
        catch /.*/
             call EchomLog('wince-common', 'WinceCommonReopenUberwins failed to open ', grouptype.grouptypename, ' uberwin group:')
             call EchomLog('wince-common', 'debug', v:throwpoint)
             call EchomLog('wince-common', 'warning', v:exception)
...
function! WinceCommonCloseSubwins(supwinid, grouptypename)
    call EchomLog('wince-common', 'debug', 'WinceCommonCloseSubwins ', a:supwinid, ':', a:grouptypename)
    let preunfix = WinceStateUnfixDimensions(a:supwinid)
    try
        if WinceModelSubwinGroupHasAfterimagedSubwin(a:supwinid, a:grouptypename)
            call EchomLog('wince-common', 'debug', 'Subwin group ', a:supwinid, ':', a:grouptypename, ' is partially afterimaged. Stomping subwins individually.')
            for subwinid in WinceModelSubwinIdsByGroupTypeName(
           \    a:supwinid,
           \    a:grouptypename
           \)
                call EchomLog('wince-common', 'verbose', 'Stomp subwin ', subwinid)
            ...
            call EchomLog('wince-common', 'debug', 'Subwin group ', a:supwinid, ':', a:grouptypename, ' is not afterimaged. Closing via toClose')
...
function! WinceCommonOpenSubwins(supwinid, grouptypename)
    call EchomLog('wince-common', 'debug', 'WinceCommonOpenSubwins ', a:supwinid, ':', a:grouptypename)
    ...
    try
        ...
        call EchomLog('wince-common', 'verbose', 'Opened subwin group ', a:supwinid, ':', a:grouptypename, ' with winids ', winids)
    finally
        call WinceStateRefixDimensions(a:supwinid, preunfix)
    endtry
    return winids
...
function! WinceCommonCloseSubwinsWithHigherPriority(supwinid, priority)
    call EchomLog('wince-common', 'debug', 'WinceCommonCloseSubwinsWithHigherPriority ', a:supwinid, ' ', a:priority)
   ...
   \)
    ...
    for grouptypename in reverse(copy(grouptypenames))
        call EchomLog('wince-common', 'verbose', 'Subwin group ', grouptypename, ' has higher priority')
...
function! WinceCommonReopenSubwins(supwinid, preserved)
    call EchomLog('wince-common', 'debug', 'WinceCommonReopenSubwins ', a:supwinid)
    for grouptype in a:preserved
        call EchomLog('wince-common', 'debug', 'Reopening preserved supwin group ', a:supwinid, ':', grouptype.grouptypename)
        try
            let winids = WinceCommonOpenSubwins(
           \    a:supwinid,
           \    grouptype.grouptypename
           \)
            call EchomLog('wince-common', 'verbose', 'Reopened subwin group ', a:supwinid, ':', grouptype.grouptypename, ' with winids ', winids)
           ...
           \)
            ...
            call EchomLog('wince-common', 'warning', 'WinceCommonReopenSubwins failed to open ', grouptype.grouptypename, ' subwin group for supwin ', a:supwinid, ':')
            call EchomLog('wince-common', 'debug', v:throwpoint)
            call EchomLog('wince-common', 'warning', v:exception)
...
function! WinceCommonPreCloseAndReopenUberwins(grouptypename)
    call EchomLog('wince-common', 'debug', 'WinceCommonPreCloseAndReopenUberwins ', a:grouptypename)
    ...
    try
        for typename in g:wince_uberwingrouptype[a:grouptypename].typenames
           ...
           \})
            ...
            call EchomLog('wince-common', 'verbose', 'Preserved uberwin ', a:grouptypename, ':', typename, ' with winid ', winid, ': ', preserved[typename])
        endfor
    finally
...
function! WinceCommonPostCloseAndReopenUberwins(grouptypename, preserved)
    call EchomLog('wince-common', 'debug', 'WinceCommonPostCloseAndReopenUberwins ', a:grouptypename)
    call WinceModelAssertUberwinGroupExists(a:grouptypename)
    let curwinid = WinceStateGetCursorWinId()
    try
        for typename in keys(a:preserved)
           ...
           \    'typename': typename
           \})
            call EchomLog('wince-common', 'verbose', 'Restore uberwin ', a:grouptypename, ':', typename, ' with winid ', winid, ': ', a:preserved[typename])
...
function! WinceCommonPreCloseAndReopenSubwins(supwinid, grouptypename)
    call EchomLog('wince-common', 'debug', 'WinceCommonPreCloseAndReopenSubwins ', a:supwinid, ':', a:grouptypename)
    ...
    let curwinid = WinceStateGetCursorWinId()
        for typename in g:wince_subwingrouptype[a:grouptypename].typenames
           ...
           \})
            let preserved[typename] = WinceStatePreCloseAndReopen(winid)
            call EchomLog('wince-common', 'verbose', 'Preserved subwin ', a:supwinid, ':', a:grouptypename, ':', typename, ' with winid ', winid, ': ', preserved[typename])
        endfor
    call WinceStateMoveCursorToWinidSilently(curwinid)
    return preserved
endfunction
...
function! WinceCommonPostCloseAndReopenSubwins(supwinid, grouptypename, preserved)
    call EchomLog('wince-common', 'debug', 'WinceCommonPostCloseAndReopenSubwins ', a:supwinid, ':', a:grouptypename)
    call WinceModelAssertSubwinGroupExists(a:supwinid, a:grouptypename)
    let curwinid = WinceStateGetCursorWinId()
        for typename in keys(a:preserved)
            let winid = WinceModelIdByInfo({
           ...
           \    'typename': typename
           \})
            call EchomLog('wince-common', 'verbose', 'Restore subwin ', a:supwinid, ':', a:grouptypename, ':', typename, ' with winid ', winid, ': ', a:preserved[typename])
            call WinceStatePostCloseAndReopen(winid, a:preserved[typename])
        endfor
    call WinceStateMoveCursorToWinidSilently(curwinid)
endfunction
...
function! WinceCommonCloseAndReopenSubwinsWithHigherPriorityBySupwin(supwinid, priority)
    call EchomLog('wince-common', 'debug', 'WinceCommonCloseAndReopenSubwinsWithHigherPriorityBySupwin ', a:supwinid, ', ', a:priority)
    let preserved = WinceCommonCloseSubwinsWithHigherPriority(a:supwinid, a:priority)
    call EchomLog('wince-common', 'verbose', 'Preserved subwins across close-and-reopen: ', preserved)
    call WinceCommonReopenSubwins(a:supwinid, preserved)

    let dims = WinceStateGetWinDimensions(a:supwinid)
    call EchomLog('wince-common', 'verbose', 'New dimensions of closed-and-reopened subwins: ', dims)
...
function! WinceCommonCloseAndReopenAllShownSubwinsBySupwin(supwinid)
    call EchomLog('wince-common', 'debug', 'WinceCommonCloseAndReopenAllShownSubwinsBySupwin ', a:supwinid)
...
function! WinceCommonAfterimageSubwinsByInfo(supwinid, grouptypename)
    call EchomLog('wince-common', 'debug', 'WinceCommonAfterimageSubwinsByInfo ', a:supwinid, ':', a:grouptypename)
    ...
    for typeidx in range(len(g:wince_subwingrouptype[a:grouptypename].typenames))
       ...
       \   !WinceModelSubwinIsAfterimaged(a:supwinid, a:grouptypename, typename)
            call EchomLog('wince-common', 'debug', 'Subwin ', a:supwinid, ':', a:grouptypename, ':', typename, ' needs afterimaging')
            let afterimagingneeded = 1
            break
        endif
        call EchomLog('wince-common', 'verbose', 'Subwin ', a:supwinid, ':', a:grouptypename, ':', typename, ' is already afterimaged')
    endfor
    if !afterimagingneeded
        call EchomLog('wince-common', 'debug', 'Subwin group ', a:supwinid, ':', a:grouptypename, ' already contains only afterimaged subwins')
        return
    endif
    ...
    for typeidx in range(len(g:wince_subwingrouptype[a:grouptypename].typenames))
        ...
        if !g:wince_subwingrouptype[a:grouptypename].afterimaging[typeidx]
            call EchomLog('wince-common', 'verbose', 'Subwin type ', a:grouptypename, ':', g:wince_subwingrouptype[a:grouptypename].typenames[typeidx], ' does not afterimage')
            continue
        endif
        ...
        if WinceModelSubwinIsAfterimaged(a:supwinid, a:grouptypename, typename)
            call EchomLog('wince-common', 'debug', 'Subwin ', a:supwinid, ':', a:grouptypename, ':', typename, ' is already afterimaged')
            continue
        endif
       ...
       \})
        ...
        call EchomLog('wince-common', 'debug', 'Afterimaging subwin ', a:supwinid, ':', a:grouptypename, ':', typename, ' with winid ', subwinid)
...
function! WinceCommonAfterimageSubwinsBySupwin(supwinid)
    call EchomLog('wince-common', 'debug', 'WinceCommonAfterimageSubwinsBySupwin ', a:supwinid)
...
function! WinceCommonAfterimageSubwinsBySupwinExceptOne(supwinid, excludedgrouptypename)
    call EchomLog('wince-common', 'debug', 'WinceCommonAfterimageSubwinsBySupwinExceptOne ', a:supwinid, ':', a:excludedgrouptypename)
    for grouptypename in WinceModelShownSubwinGroupTypeNamesBySupwinId(a:supwinid)
...
function! WinceCommonDeafterimageSubwinsBySupwin(supwinid)
    call EchomLog('wince-common', 'debug', 'WinceCommonDeafterimageSubwinsBySupwin ', a:supwinid)
    for grouptypename in WinceModelShownSubwinGroupTypeNamesBySupwinId(a:supwinid)
        call EchomLog('wince-common', 'verbose', 'Subwin group ', a:supwinid, ':', grouptypename)
        if WinceModelSubwinGroupHasAfterimagedSubwin(a:supwinid, grouptypename)
            call EchomLog('wince-common', 'debug', ' Closing-and-reopening subwin groups of supwin ', a:supwinid, ' starting with partially afterimaged group ', grouptypename)
...
function! WinceCommonUpdateAfterimagingByCursorWindow(curwin)
    call EchomLog('wince-common', 'debug', 'WinceCommonUpdateAfterimagingByCursorWindow ', a:curwin)
    ...
    if finalpos.category ==# 'uberwin' || finalpos.category ==# 'none'
        call EchomLog('wince-common', 'debug', 'Cursor position is uberwin. Afterimaging all subwins of all supwins.')
        for supwinid in WinceModelSupwinIds()
    ...
    elseif finalpos.category ==# 'supwin'
        call EchomLog('wince-common', 'debug', 'Cursor position is supwin ', finalpos.id, '. Afterimaging all subwins of all other supwins')
        for supwinid in WinceModelSupwinIds()
            if supwinid !=# finalpos.id
                call WinceCommonAfterimageSubwinsBySupwin(supwinid)
            endif
        endfor
        call EchomLog('wince-common', 'debug', 'Cursor position is supwin ', finalpos.id, '. Deafterimaging all its subwins. ')
    ...
    elseif finalpos.category ==# 'subwin'
        call EchomLog('wince-common', 'debug', 'Cursor position is subwin ', finalpos.supwin, ':', finalpos.grouptype, ':', finalpos.typename, '. Afterimaging all subwins of all other supwins.')
        for supwinid in WinceModelSupwinIds()
            if supwinid !=# finalpos.supwin
                call WinceCommonAfterimageSubwinsBySupwin(supwinid)
            endif
        endfor
        call EchomLog('wince-common', 'debug', 'Cursor position is subwin ', finalpos.supwin, ':', finalpos.grouptype, ':', finalpos.typename, '. Afterimaging all subwins of supwin ', finalpos.supwin, ' except those in group ', finalpos.supwin, ':', finalpos.grouptype)
...
function! s:DoWithout(curwin, callback, args, nouberwins, nosubwins, reselect, preservesupdims)
    call EchomLog('wince-common', 'debug', 'DoWithout ', a:curwin, ', ', a:callback, ', ', a:args, ', [', a:nouberwins, ',', a:nosubwins, ',', a:reselect, ',', a:preservesupdims, ']')
    ...
    call EchomLog('wince-common', 'debug', 'Cursor position before removals is ', info)
    ...
    if a:nosubwins && !empty(supwinids)
        ...
        if a:curwin.category ==# 'supwin'
            call EchomLog('wince-common', 'verbose', 'Cursor is in supwin ', a:curwin.id, '. Its subwins will be closed first')
            let startwith = str2nr(a:curwin.id)

        " If the cursor is in a subwin, start with its supwin
        elseif a:curwin.category ==# 'subwin'
            call EchomLog('wince-common', 'verbose', 'Cursor is in subwin ', a:curwin.supwin, ':', a:curwin.grouptype, ':', a:curwin.typename, '. Subwins of supwin ', a:curwin.supwin, ' will be closed first')
        ...
        call EchomLog('wince-common', 'debug', 'Closing all subwins')
        for supwinid in supwinids
             let closedsubwingroupsbysupwin[supwinid] = 
    ...
    try
        ...
        if a:nouberwins
            call EchomLog('wince-common', 'debug', 'Closing all uberwins')
        ...
        try
            if type(a:curwin) ==# v:t_dict
                ...
                if WinceStateWinExists(winid)
                    call EchomLog('wince-common', 'debug', 'Cursor window still exists. Moving to it.')
                    call WinceStateMoveCursorToWinid(winid)
                endif
            endif
            call EchomLog('wince-common', 'verbose', 'Invoking callback ', a:callback, ' with args ', a:args)
            let retval = call(a:callback, a:args)
            call EchomLog('wince-common', 'verbose', 'Callback gave return value ', retval)
            let info = WinceCommonGetCursorPosition()
            call EchomLog('wince-common', 'debug', 'New cursor position after callback is ', info)
            let posttabnr = WinceStateGetTabnr()
            call EchomLog('wince-common', 'debug', 'New tab after callback is ', posttabnr)

        finally
            if pretabnr !=# posttabnr
                call EchomLog('wince-common', 'debug', 'Callback changed tab from ', pretabnr, ' to ', posttabnr, '. Returning before reopening.')
                call WinceStateGotoTab(pretabnr)
            endif
            call EchomLog('wince-common', 'debug', 'Reopening all uberwins')
        ...
        call EchomLog('wince-common', 'debug', 'Reopening all subwins')
        for supwinid in supwinids
            ...
            if !WinceStateWinExists(supwinid)
                if pretabnr !=# posttabnr
                    ...
                    let gotopretabnr = 1
                    call EchomLog('wince-common', 'debug', 'Supwin ', supwinid, ' has moved to tab ', posttabnr, ' and changed winid to ', newsupwinid, '. Restoring subwins there.')
                else
                    call EchomLog('wince-common', 'debug', 'Supwin ', supwinid, ' has vanished from state. Not restoring its subwins.')
                    continue
                endif
            endif
            call EchomLog('wince-common', 'verbose', 'Reopening all subwins of supwin ', newsupwinid)
            ...
            call EchomLog('wince-common', 'verbose', 'Supwin dimensions after reopening its subwins: ', dims)
            " Afterimage everything after finishing with each supwin to avoid collisions
            call EchomLog('wince-common', 'verbose', 'Afterimaging all reopened subwins of supwin ', newsupwinid, ' to avoid collisions with the next supwin')
            ...
            if gotopretabnr
                call EchomLog('wince-common', 'debug', 'Finished restoring subwins of supwin ', newsupwinid, ' in tab ', posttabnr, '. Returning to tab ', pretabnr)
                call WinceStateGotoTab(pretabnr)
            endif
        endfor
        if pretabnr !=# posttabnr
            call EchomLog('wince-common', 'debug', 'Finished reopening. All afterimaging subwins afterimaged. Moving back to post-callback tab ', posttabnr)
            call WinceStateGotoTab(posttabnr)
        endif
        if a:reselect
...
function! WinceCommonDoWithoutUberwins(curwin, callback, args, reselect)
    call EchomLog('wince-common', 'debug', 'WinceCommonDoWithoutUberwins ', a:curwin, ', ', a:callback, ', ', a:args, ', ', a:reselect)
...
function! WinceCommonDoWithoutSubwins(curwin, callback, args, reselect)
    call EchomLog('wince-common', 'debug', 'WinceCommonDoWithoutSubwins ', a:curwin, ', ', a:callback, ', ', a:args, ', ', a:reselect)
...
function! WinceCommonDoWithoutUberwinsOrSubwins(curwin, callback, args, reselect, preservesupdims)
    call EchomLog('wince-common', 'debug', 'WinceCommonDoWithoutUberwinsOrSubwins ', a:curwin, ', ', a:callback, ', ', a:args, ', ', a:reselect, ', ', a:preservesupdims)
    return s:DoWithout(a:curwin, a:callback, a:args, 1, 1, a:reselect, a:preservesupdims)
endfunction

function! s:Nop()
    call EchomLog('wince-common', 'debug', 'Nop')
endfunction
...
function! WinceCommonCloseAndReopenAllShownSubwins(curwin)
    call EchomLog('wince-common', 'debug', 'WinceCommonCloseAndReopenAllShownSubwins ', a:curwin)
...
function! WinceCommonSubwinFlagStrByGroup(grouptypename)
    call EchomLog('wince-common', 'debug', 'WinceCommonSubwinFlagStrByGroup ', a:grouptypename)
